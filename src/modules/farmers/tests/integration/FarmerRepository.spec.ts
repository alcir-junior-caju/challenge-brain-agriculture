...
describe('FarmerRepository Integration Tests', () => {
  ...
  it('should be get a farmer', async () => {
    ...
    expect(output.id).toBe(farmerEntity.id.value)
    expect(output.name).toBe(farmerEntity.name.value)
    expect(output.email).toBe(farmerEntity.email.value)
    expect(output.document).toBe(farmerEntity.document.value)
  ...
  it('should be update a farmer', async () => {
    ...
    const output = await farmerRepository.find(farmerEntity.id.value)
    expect(output.id).toBe(farmerUpdatedEntity.id.value)
    expect(output.name).toBe(farmerUpdatedEntity.name.value)
    expect(output.email).toBe(farmerUpdatedEntity.email.value)
    expect(output.document).toBe(farmerUpdatedEntity.document.value)
    await farmerRepository.delete(farmerEntity.id.value)
  })
})
